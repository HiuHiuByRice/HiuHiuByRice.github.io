...
export function matchesCategoryBranch(
    category: string | string[] | null | undefined,
    branch: string | string[],
): boolean {
    const categoryParts = getCategoryPathParts(category);
    const branchParts = getCategoryPathParts(branch);

    if (!categoryParts || !branchParts || branchParts.length > categoryParts.length) {
        return false;
    }

    return branchParts.every((part, index) => categoryParts[index] === part);
}
