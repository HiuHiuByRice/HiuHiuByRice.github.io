<script lang="ts">
    ...
    function hasActiveDescendant(node: DirectoryNode): boolean {
        if (node.url && isPathActive(node.url)) return true;
    ...
    function autoExpand(nodes: DirectoryNode[], parentPath: string): boolean {
        ...
        for (const node of nodes) {
            ...
            if (node.type === "file") {
                ...
                const selfActive = isPathActive(node.url);
...
{#snippet renderNode(node: DirectoryNode, path: string, depth: number)}
    ...
    <div class="flex flex-col">
        {#if node.type === "folder"}
            <div
                class="min-w-0 flex items-center px-2 py-1.5 my-0.5 rounded-md transition-colors duration-300 hover:bg-(--btn-plain-bg-hover) {hasActiveDescendant(node) ? 'bg-(--btn-plain-bg-hover) text-(--primary)' : expandedFolders[currentPathStr] ? 'font-medium text-90' : 'font-medium text-75'}"
            >
                {#if node.url}
                    <a
                        href={node.url}
                        class="min-w-0 flex items-center flex-1 hover:text-(--primary)"
                    >
                ...
                <div
                    class="flex items-center justify-center ml-1.5 w-4 h-4 cursor-pointer transition-transform duration-300 {expandedFolders[currentPathStr] ? 'rotate-90' : ''}"
